...
export class TeaPieLanguageServer {
    ...
    public async initialize(): Promise<void> {
        try {
            ...
            console.log('[TeaPie] Initializing TeaPie Language Server...');
            await this.loadXmlDocumentation();
            
            if (this.xmlDocs.length > 0) {
                console.log(`[TeaPie] XML documentation loaded with ${this.xmlDocs.length} members`);
            } else {
                console.log('[TeaPie] Failed to load XML documentation');
            }
                ...
                vscode.languages.registerCompletionItemProvider(
                    { scheme: 'file', pattern: '**/*.csx' },
                    {
                        provideCompletionItems: async (
                            ...
                            console.log(`[TeaPie] Getting completions for line: "${linePrefix}"`);

                            // Handle empty completions
                            if (this.xmlDocs.length === 0) {
                                console.log('[TeaPie] No XML documentation available, attempting to load...');
                                await this.loadXmlDocumentation();
                                
                                if (this.xmlDocs.length === 0) {
                                    console.log('[TeaPie] Still no documentation available, returning default completions');
                            ...
                            if (completionItems.length === 0) {
                                console.log('[TeaPie] No specific completions found, letting OmniSharp handle it');
                                return undefined;
                            }

                            console.log(`[TeaPie] Returning ${completionItems.length} completion items`);
            ...
            console.log('[TeaPie] TeaPie Language Server initialized successfully');
        } catch (error) {
            console.error('[TeaPie] Failed to initialize TeaPie Language Server:', error);
            throw error;
        }
    }
    ...
    public async loadXmlDocumentation(): Promise<void> {
        try {
            ...
            console.log('[TeaPie] Searching for TeaPie XML documentation in locations:', possibleLocations);

            let allDocs: XmlDocMember[] = [];

            for (const baseLocation of possibleLocations) {
                console.log(`[TeaPie] Checking location: ${baseLocation}`);
                
                if (fs.existsSync(baseLocation)) {
                    console.log(`[TeaPie] Location exists: ${baseLocation}`);
                    ...
                    const searchDir = async (dir: string): Promise<string[]> => {
                        try {
                            ...
                            console.error(`[TeaPie] Error reading directory ${dir}:`, err);
                            return [];
                        }
                    };

                    const files = await searchDir(baseLocation);
                    console.log(`[TeaPie] Found ${files.length} files in directory`);
                    ...
                    console.log(`[TeaPie] Found ${xmlFiles.length} potential XML documentation files:`, xmlFiles);
                    
                    // Try to load documentation from each file
                    for (const xmlFile of xmlFiles) {
                        console.log(`[TeaPie] Loading documentation from: ${xmlFile}`);
                        try {
                            const docs = await XmlDocParser.loadXmlDocs(xmlFile);
                            if (docs.length > 0) {
                                console.log(`[TeaPie] Successfully loaded ${docs.length} documentation members from ${xmlFile}`);
                                allDocs = [...allDocs, ...docs];
                            } else {
                                console.log(`[TeaPie] No documentation members found in ${xmlFile}`);
                            }
                        } catch (error) {
                            console.error(`[TeaPie] Failed to load documentation from ${xmlFile}:`, error);
                        }
                    }
                    ...
                    console.log(`[TeaPie] Location does not exist: ${baseLocation}`);
                }
            }
            ...
            if (this.xmlDocs.length === 0) {
                console.log('[TeaPie] No XML documentation found in any of the searched locations');
            } else {
                console.log(`[TeaPie] Successfully loaded ${this.xmlDocs.length} total documentation members`);
                // Log first few members to verify content
                console.log('[TeaPie] Sample members:', this.xmlDocs.slice(0, 3).map(m => m.name));
                
                // Check for TeaPie.TeaPie members specifically
                const teaPieMembers = XmlDocParser.getMembersByType(this.xmlDocs, 'TeaPie.TeaPie');
                console.log(`[TeaPie] Found ${teaPieMembers.length} TeaPie.TeaPie members`);
                if (teaPieMembers.length > 0) {
                    console.log('[TeaPie] Sample TeaPie members:', teaPieMembers.slice(0, 3).map(m => m.name));
                }
            }
        } catch (error) {
            console.error('[TeaPie] Failed to load XML documentation:', error);
            this.xmlDocs = [];
        }
    }
    ...
    private createCompletionItem(member: XmlDocMember): vscode.CompletionItem {
        ...
        if (this.teaPieMethodMap[member.name]) {
            ...
            console.log(`[TeaPie] Using explicit name mapping for ${member.name} -> ${displayName}`);
        }
        // Handle different member types if not in our map
            ...
            if (member.name.includes('(TeaPie.TeaPie,') || member.name.includes('(TeaPie.TeaPie)')) {
                ...
                if (methodNameMatch && methodNameMatch[1]) {
                    ...
                    console.log(`[TeaPie] Extracted extension method name "${displayName}" from ${member.name}`);
                } else {
                    // Fallback for extension methods
                    const parts = member.name.split('.');
                    const methodPart = parts[parts.length - 1];
                    displayName = methodPart.split('(')[0].replace(/^M:/, '');
                    console.log(`[TeaPie] Fallback extension method name "${displayName}" from ${member.name}`);
                }
            } 
            // Regular methods
            else {
                ...
                console.log(`[TeaPie] Extracted method name "${displayName}" from ${member.name}`);
            }
            ...
            console.log(`[TeaPie] Extracted property name "${displayName}" from ${member.name}`);
        } else {
            ...
            console.log(`[TeaPie] Extracted other name "${displayName}" from ${member.name}`);
        }
        
        console.log(`[TeaPie] Final display name for ${member.name} is "${displayName}"`);
    ...
    private getDefaultCompletions(): vscode.CompletionList {
        console.log('[TeaPie] Providing default TeaPie completions');
    ...
    private async getTeaPieMembers(): Promise<vscode.CompletionItem[]> {
        console.log('[TeaPie] Getting TeaPie members for completion');
        ...
        console.log(`[TeaPie] Found ${teaPieMembers.length} TeaPie members after filtering`);
        teaPieMembers.forEach(m => console.log(`[TeaPie] Included member: ${m.name}`));
